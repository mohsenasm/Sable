...
export function RoomProfileEdit({
    ...
    useCallback(
      async (
        ...
        if (roomName !== undefined) {
          await mx.sendStateEvent(
            room.roomId,
            EventType.RoomName as keyof StateEvents,
            {
              name: roomName,
            },
          );
        ...
        if (roomTopic !== undefined) {
          await mx.sendStateEvent(
            room.roomId,
            EventType.RoomTopic as keyof StateEvents,
            {
              topic: roomTopic,
            },
          );
  ...
  const handleSubmit: FormEventHandler<HTMLFormElement> = (evt) => {
    ...
    const topicTextArea = target?.topicTextArea as
      | HTMLTextAreaElement
      | undefined;
    ...
    <Box as="form" onSubmit={handleSubmit} direction="Column" gap="400">
      ...
      <Box gap="300">
        <Button
          ...
          before={
            submitting && <Spinner size="100" variant="Success" fill="Solid" />
          }
        >
...
export function RoomProfile({ permissions }: RoomProfileProps) {
  ...
  const avatar = useRoomAvatar(
    room,
    directs.has(room.roomId) && !customDMCards,
  );
  const name = useRoomName(room);
  const topic = useRoomTopic(room);
  const joinRule = useRoomJoinRule(room);

  const canEditAvatar = permissions.stateEvent(
    EventType.RoomAvatar,
    mx.getSafeUserId(),
  );
  const canEditName = permissions.stateEvent(
    EventType.RoomName,
    mx.getSafeUserId(),
  );
  const canEditTopic = permissions.stateEvent(
    EventType.RoomTopic,
    mx.getSafeUserId(),
  );
